...
public class TDView extends SurfaceView implements Runnable {
    ...
    private boolean firstboost = true;

    float mBoostVolume = 1;
        ...
        if (sm.soundisloaded)
        {
            if (!gameEnded)
            {
                ...
                else
                {
                    ...
                    if (mBoostVolume > 0.01f)
                    {
                        mBoostVolume *= 0.75f;
                        sm.setVolumeBoost(mBoostVolume);
                        Log.d("volume", "boost decreasing");
                    }
                    else
                    {
                        mBoostVolume = 0f;
    ...
    private void draw()
    {
        if (ourHolder.getSurface().isValid())
        {
            ...
            else
            {
                ...
                paint.setTextSize(80);
                paint.setTextAlign(Paint.Align.CENTER);
                canvas.drawText("Loading", screenX / 2, 100, paint);
            }
            paint.setTextSize(25);
